...
| 功能 | 描述 | 优先级 |
|------|------|--------|
| 关卡定义 | YAML/JSON 格式的关卡配置文件 | P0 |
| 难度分级 | 初级/中级/高级三个难度等级 | P0 |
| 分类标签 | 按功能分类：导航、编辑、搜索、调试等 | P0 |
| 场景描述 | 每个关卡包含实际使用场景的描述 | P0 |
| 解锁机制 | 完成前置关卡后解锁后续关卡 | P1 |
| 自定义关卡 | 用户可以创建自定义关卡 | P2 |

#### 2.2.2 训练模式 (Training Modes)

| 功能 | 描述 | 优先级 |
|------|------|--------|
| 挑战模式 | 按顺序完成关卡，类似 Githug | P0 |
| 自由练习 | 可选择任意已解锁关卡练习 | P1 |
| 复习模式 | 基于遗忘曲线的间隔重复复习 | P2 |
| 限时模式 | 在限定时间内完成挑战 | P2 |
| 随机模式 | 随机抽取已解锁关卡进行测试 | P2 |

#### 2.2.3 键位配置 (Keymap Profiles)

| 功能 | 描述 | 优先级 |
|------|------|--------|
| VSCode 默认 | 支持 VSCode 原生快捷键 | P0 |
| VSCode + Vim | 支持 Vim 扩展的额外快捷键 | P0 |
| 平台适配 | 自动识别 macOS/Windows/Linux | P0 |
| 自定义映射 | 允许用户导入自定义 keybindings.json | P2 |
